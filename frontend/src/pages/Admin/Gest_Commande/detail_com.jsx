  ...
  if (loading) {
    return (
      <div className="min-h-screen w-full flex items-center justify-center gap-2 text-gray-500">
        ...
        <span className="font-sans text-xs uppercase tracking-wider font-bold">Extraction des données de transaction...</span>
      </div>
    );
  }
  ...
  if (error || !commande) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-white p-6 text-center font-[Cambria,Cochin,Georgia,Times,'Times_New_Roman',serif]">
        ...
        <p className="text-xs font-sans text-gray-400 mb-6">{error || "Aucun enregistrement trouvé."}</p>
        <button 
    ...
    <div className="min-h-screen bg-white font-[Cambria,Cochin,Georgia,Times,'Times_New_Roman',serif] px-4 md:px-8 py-12 pb-20 w-full box-border flex flex-col gap-8">
      ...
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center w-full gap-4">
        <div className="flex flex-col gap-1">
          <h1 className="text-2xl md:text-3xl font-bold tracking-tighter text-gray-900 m-0 break-all">
        ...
        <button 
          className="flex items-center gap-2 text-xs font-bold bg-gray-100 hover:bg-gray-200 text-gray-800 px-4 py-3 rounded-xl transition-all duration-200 cursor-pointer border-none active:scale-95 uppercase tracking-wider shrink-0" 
          onClick={retourCommande}
        >
          <ArrowLeft size={14} />
          Retour à la liste
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 items-stretch">
        <div className="bg-white border border-gray-100 rounded-2xl p-6 md:p-8 grid grid-cols-1 sm:grid-cols-3 gap-6 shadow-xl items-center">
          <div className="flex flex-col gap-1">
            <span className="text-[10px] font-bold tracking-wider text-gray-400 uppercase">Client & Destinataire</span>
            <h3 className="text-base font-bold m-0 text-gray-900">{commande.nom_destinataire}</h3>
            <p className="text-xs text-gray-400 m-0 font-sans mt-0.5">{commande.email_contact || "Aucun email fourni"}</p>
            <p className="text-[11px] text-gray-500 m-0 font-sans">{commande.telephone_contact}</p>
          </div>
          
          <div className="flex flex-col gap-1">
            <span className="text-[10px] font-bold tracking-wider text-gray-400 uppercase">Adresse de Livraison</span>
            <h3 className="text-sm font-bold m-0 text-gray-900">{commande.adresse_livraison || "Non spécifiée"}</h3>
            <p className="text-[10px] text-gray-400 m-0 uppercase font-sans mt-1">Canal: {commande.canal_commande}</p>
          </div>
        ...
        <div className="bg-gray-900 text-white rounded-2xl p-6 shadow-xl flex flex-col justify-between gap-4">
          <div>
            ...
            <h2 className="text-2xl font-bold m-0 mt-1 font-sans">
              {Intl.NumberFormat('fr-FR').format(commande.total_commande || 0)} <span className="text-xs font-serif font-medium text-gray-400">FCFA</span>
            </h2>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shadow-xl">
        <div className="flex flex-col">
          <h3 className="text-sm font-bold m-0 text-gray-900">
            Modifier le statut de l'accès {updating && <span className="text-xs text-gray-400 font-normal">(Mise à jour...)</span>}
          </h3>
          <p className="text-xs text-gray-400 m-0 mt-0.5 font-sans">Cette action mettra à jour l'état de la commande et le calcul du chiffre d'affaires.</p>
        </div>
        ...
        <div className="flex gap-2 flex-wrap font-sans">
          <button 
            ...
            className={`px-4 py-2 text-[10px] font-bold uppercase tracking-wider rounded-xl cursor-pointer transition-all border-none ${commande.statut_commande?.toUpperCase() === "EN COURS" ? "bg-amber-500 text-white" : "bg-amber-5 border-none text-amber-700 hover:bg-amber-100"}`}
          >
            En cours
          </button>
          <button 
            ...
            className={`px-4 py-2 text-[10px] font-bold uppercase tracking-wider rounded-xl cursor-pointer transition-all border-none ${commande.statut_commande?.toUpperCase() === "PAYEE" ? "bg-[#1A4301] text-white" : "bg-[#9ADE7B]/20 text-[#1A4301] hover:bg-[#9ADE7B]/30"}`}
          >
            Payée
          </button>
          <button 
            ...
            className={`px-4 py-2 text-[10px] font-bold uppercase tracking-wider rounded-xl cursor-pointer transition-all border-none ${commande.statut_commande?.toUpperCase() === "ANNULE" ? "bg-red-600 text-white" : "bg-red-50 text-red-600 hover:bg-red-100"}`}
          >
            Annulée
          </button>
        </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-xl flex flex-col">
        <div className="w-full overflow-x-auto">
          <table className="w-full border-collapse text-left">
            <thead>
              <tr className="bg-gray-50 border-b border-b-gray-100">
            ...
            <tbody className="divide-y divide-gray-50">
              {currentItems.length > 0 ? (
                currentItems.map((detail) => {
                  ...
                  // Extraction de la source de l'image calquée exactement sur ce qui fonctionne dans Gest_produit
                  // On vérifie d'abord si l'image est dans la relation produit, sinon directement dans la ligne
                  const imageCible = detail.produit?.url_image_principale || detail.url_image_principale || detail.image;
                  
                  // On applique l'adresse par défaut d'Unsplash tirée de ton Gest_produit s'il n'y a rien
                  ...
                  return (
                    <tr key={detail.id_detail_ligne} className="hover:bg-gray-50/50 transition-colors">
                      <td className="p-4 flex items-center gap-4">
                        <img 
                          src={imageSource} 
                          alt={nomProduit} 
                          className="w-12 h-12 object-cover rounded-xl border border-gray-100 bg-gray-50 shrink-0 shadow-sm"
                        />
                        <div className="flex flex-col">
                          <h4 className="text-sm font-bold m-0 text-gray-900">{nomProduit}</h4>
                          <p className="text-[10px] font-bold uppercase text-gray-400 m-0 font-sans mt-0.5">
                            {descriptionProduit}
                          </p>
                        </div>
                      </td>
                      <td className="p-4 text-sm font-bold text-gray-800 text-center font-sans">{detail.quantite_commandee}</td>
                      <td className="p-4 text-sm font-bold text-gray-500 text-right font-sans">
                        {Intl.NumberFormat('fr-FR').format(prixUnitaireBrut)} <span className="text-[10px] font-serif font-medium text-gray-400">FCFA</span>
                      </td>
                      <td className="p-4 text-sm font-bold text-gray-900 text-right font-sans">
                        {Intl.NumberFormat('fr-FR').format(detail.prix_global_scelle || 0)} <span className="text-[10px] font-serif font-medium text-gray-400">FCFA</span>
                      </td>
                    </tr>
                  );
                })
